...
module SDRCFunctionsVar
  ...
  double precision :: tempconsb=0.1,tempconshd=0.1,tempconshn=0.1
  double precision :: tempconsed=0.1,tempconsen=0.1,tempconshn2=0.1
  integer, parameter :: tt=55,tmptt=1100
	...
	integer, parameter :: samplen=978, d=1, p=9, p1=d*(p-d),lw=p1*(p1*3+13),lwt = 2*(2*3+13)
...
implicit none
	integer :: eflag, i, j, t, e, ii, ind 
  integer :: a1,a2,i1,i2,i3,i4
	double precision, dimension(2) :: tempx, fvalx
	double precision :: temp, ntemp, xstd, c, criterion, vic, tmprand
  ...
  double precision, dimension(d*(p-d),d*(p-d)) :: estVarMat
	...
	double precision, dimension(tmptt*p,samplen) :: bbigx
	double precision, dimension(tmptt,samplen) :: bbigz
	double precision, dimension(tmptt,samplen) :: bbigd
  ...
  double precision, allocatable, dimension(:) :: bandhin,bandbin,bandein, vec1, vec2
  double precision, allocatable, dimension(:,:) :: paraExp1, paraExp2

    allocate(paraExp1((p-d-1),d+1), paraExp2((p-d-1),d+1))
    allocate(vec1(p-1-d), vec2(p-1-d))
    vec1 = 1.0
    vec2 = 0.0!

	call getx(tmptt,p,samplen,bbigx)
	call getz(tmptt,samplen,bbigz)
	call getd(tmptt,samplen,bbigd)

  ind=14 ! need to change
  i1=(ind-1)*(tt*p)+1
  i2=ind*tt*p 
  i3=(ind-1)*tt+1
  i4=ind*tt
  ...
  open(1,file='bs14_est.txt')
!  open(2,file='real_var.txt')
...
!      do i = 1,d
!      do j = 1,(p-d)
!        parasTrue(j,i) = parasInitial((i-1)*p+d+j)
!      enddo
!      enddo
  ...
  call init_random_seed() ! by SC
    do i = 1,p-d
      do j = 1,d
        call RANDOM_NUMBER(tmprand) ! by SC
        paras0(i,j) = parasTrue(i,j) + (tmprand-0.5)*2.!/4.!(epsilonval-0.5)*2 ! by SC
      enddo
    enddo
...
!   end do
! calculate vic
!   call scoreOneExp((d+1)*(p-d-1),paraExp1,scoreValnew1,eflag)
!   call scoreOneExp((d+1)*(p-d-1),paraExp2,scoreValnew2,eflag)
!   vic = (sum(scoreValnew1*scoreValnew1)+sum(scoreValnew2*scoreValnew2))&
!            *sqrt(dble(samplen))/2.0+p*d*log(dble(samplen))
!   print*, paraExp1,paraExp2
!   print*, scoreValnew1
!   print*, scoreValnew2
!   print*, p*d*log(dble(samplen)),vic
!    print*,"vic=",vic
...
subroutine scoreOneExp(n,paras1,scoreVal,eflag)
! paras: parameter \beta in equation (5)
! xinput, deltainput, zinput: matrix X, \Delat vector, Z vector in equation (5)
! Y is indicator function which calculated in this function
! samplen: number of observations, p: dimension of all parameters, d: dimension of upper block, where beta's are 1
  ...
  use SDRCFunctionsVar
  integer :: i, j, k, n, t
  double precision, dimension(d+1):: lambdaNum,temp2,termOne,termTwo,lambdaVal
  double precision, dimension(p-d-1):: expectNum
  double precision, dimension(samplen,samplen,d+1):: btxall
  double precision, dimension(samplen,d+1)::lambdaVec,btransx,khprimevec&
    ,numeVec
  double precision, dimension(samplen,(p-d-1)):: expectVec
  double precision:: lambdaDen, expectDen, temp1
  double precision, dimension((p-d-1),d+1):: paras1
  double precision, dimension(n), intent(out):: scoreVal
  integer,intent(in) :: eflag
  double precision, dimension(samplen)::  betaXtemp2,  denoVec, val2!vector of denominator in hazard estimation
  ...
  do i = 1,samplen
    do j = 1,(d+1)
      btransx(i,j) = xall(i,j)
      do t = 1,(p-d-1)
        btransx(i,j) = btransx(i,j)+paras1(t,j)*xall(i,d+1+t)
      enddo
    enddo
  enddo
  do i = 1,samplen
  ...
  enddo
  do i = 1,samplen
    call kernel(btxall(i,:,:)/bandh/tempconshd,samplen,betaXtemp2,'norm',d+1)
    betaXtemp2 = betaXtemp2/bandh/tempconshd !kernel values
    do t = 1,samplen
      temp1 = 0.0
      do j = 1,samplen
...
!print*,"val2=",val2     
    ...
    do t = 1,(d+1)
      termOne(t) = sum(val2*deltainput*khprimevec(:,t)/denoVec)
      termTwo(t) = sum(val2*deltainput*betaXtemp2*numeVec(:,t)/&
                (denoVec*denoVec))
      lambdaNum(t) = -termOne(t)+termTwo(t)
    enddo
...
! print*,"lambda",lambdaVec
    ...
    call kernel(btxall(i,:,:)/bandhe/tempconsed,samplen,val2,'norm',d+1)
    do j = 1,samplen
      if (zdiffall(i,j)>=0) then
        betaXtemp2(j) = val2(j)
      else
    ...
    enddo
    expectDen = sum(betaXtemp2)/sum(val2)
        call kernel(btxall(i,:,:)/bandhe/tempconsen,samplen,val2,'norm',d+1)
        do j = 1,samplen
            if (zdiffall(i,j)>=0) then
                betaXtemp2(j) = val2(j)
            else
    ...
    do t = 1,(p-d-1)
      expectNum(t) = sum(betaXtemp2*xall(:,d+1+t))/(sum(val2) + eps) ! by SC
    enddo
! print*,"expectation",expectNum
    do j = 1,(p-d-1)
      expectVec(i,j) = xall(i,d+1+j)-expectNum(j)/(expectDen + eps) ! by SC
    enddo
  enddo
! print*,"expectation",expectVec
  do i = 1,(d+1)
    do j = 1,(p-d-1)
      do k = 1,samplen
        scoreVal((i-1)*(p-d-1)+j) = scoreVal((i-1)*(p-d-1)+j)+&
                    deltainput(k)*lambdaVec(k,i)*expectVec(k,j)
      enddo
    enddo
  enddo
  scoreVal = scoreVal/samplen
end subroutine scoreOneExp
