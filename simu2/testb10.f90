...
module SDRCFunctionsVar
  ...
  double precision :: tempconsb=0.02,tempconshd=0.02,tempconshn=0.02
  double precision :: tempconsed=0.02,tempconsen=0.02,tempconshn2=0.05
  integer, parameter :: tt=100,tmptt=1000
	...
	integer, parameter :: samplen=500, d=1, p=12, p1=d*(p-d),lw=p1*(p1*3+13),lwt = 2*(2*3+13)
...
implicit none
  ...
  open(1,file='testb10_est.txt')
  open(2,file='testb10_var.txt')
  ...
  call init_random_seed() ! by SC
    do i = 1,p-d
      do j = 1,d
        ...
        paras0(i,j) = parasTrue(i,j) + (tmprand-0.5)/4.!(epsilonval-0.5)*2 ! by SC
      enddo
    enddo
